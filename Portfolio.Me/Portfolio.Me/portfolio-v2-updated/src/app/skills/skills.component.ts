...
export class SkillsComponent implements AfterViewInit {
    ...
    skills: Skill[] = [];
    scrollLeft() {
        const _scrollElm: HTMLElement = document.getElementById('contentSkillsList');
    ...
    ngAfterViewInit() {
        ...
        const skillsWrapper = document.querySelector('.app-skills');
        const populateSkills = () => {
            _.each(SKILLS_ARRAY, (_item, _iter: any) => {
                const _skill: Skill = new Skill(_item['id'], _item['src'], _item['tag']);
                self.skills.push(_skill);
            });
            skillsPopulated = true;
        };
        if (skillsWrapper) {
            const checkForScrollPosition = function () {
                const props: ClientRect = skillsWrapper.getBoundingClientRect();
                if ((window.innerHeight - props.top) >= 0) {
                    populateSkills();
                }
            };
            let ticking = false;
            document.addEventListener('scroll', (e: Event) => {
                if (!ticking && !skillsPopulated) {
