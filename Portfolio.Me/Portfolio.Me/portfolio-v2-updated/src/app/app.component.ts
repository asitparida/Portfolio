...
export class AppComponent implements OnInit, AfterViewInit {
  ...
  ngAfterViewInit() {
    ...
    const _dock = document.querySelector('[data-tag-type="dock"]');
    if (_dock) {
      _dock.innerHTML = _tmpl;
      setTimeout(() => {
        const _ele: Element = document.getElementById('chrome-toggle-switch');
        _ele.addEventListener('change', (e: Event) => {
          const _checked: boolean = e.target['checked'] || false;
          if (_checked) {
            const _targetElms: NodeListOf<Element> = document.querySelectorAll('[data-tag="dark-mode-applicable"]');
            for (let i = 0; i < _targetElms.length; i++) {
              _targetElms[i].classList.add('dark');
            }
            if (_targetElms.length > 0) {
              const _textElm: Element = document.querySelector('[data-tag="chrome-toggle-text"]');
              if (_textElm) {
